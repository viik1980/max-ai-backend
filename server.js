...
const { canaryHeaderCheck, registerCanaryRoute } = require('./canary');

const app = express();
// Railway стоит перед сервисом как прокси — без этого req.ip и
// X-Forwarded-For будут показывать внутренний адрес балансировщика
// Railway, а не реального клиента. 1 — доверяем ровно одному хопу
// прокси (именно столько у Railway), не "доверяй всем" (это было бы
// небезопасно — тогда IP можно подделать самим заголовком).
app.set('trust proxy', 1);
app.use(cors());
app.use(express.json({ limit: '15mb' })); // фото в base64 бывают тяжёлые
app.use(canaryHeaderCheck); // ловушка проверяется раньше всего остального
registerCanaryRoute(app); // приманка-эндпоинт /api/admin/config
...
if (!GOOGLE_MAPS_API_KEY) {
  console.warn('⚠️  GOOGLE_MAPS_API_KEY не задан — поиск точек (📍) в приложении работать не будет.');
}
...
// -----------------------------------------------------------------
// Список разрешённых e-mail — закрытый доступ для конкретных людей,
// а не просто "любой, у кого есть APK". Firebase-токен сам по себе
// подтверждает только подлинность приложения, не личность человека —
// этот список добавляет второй, более узкий барьер.
//
// E-mail, а не uid: привязан к Google-аккаунту человека, а не к
// конкретной установке приложения — переживает сброс телефона,
// переустановку, покупку нового устройства без переприглашения.
//
// Формат в Variables на Railway: через запятую, без пробелов —
//   ALLOWED_EMAILS=ivan@gmail.com,petr@gmail.com
// Сравнение регистронезависимое.
// Если переменная не задана — доступ открыт всем с валидным токеном
// (старое поведение); удобно для разработки, НЕ оставлять так в проде.
// -----------------------------------------------------------------
...
if (ALLOWED_EMAILS.size === 0) {
  console.warn('⚠️  ALLOWED_EMAILS пуст — доступ открыт ЛЮБОМУ с валидным Firebase-токеном.');
} else {
  console.log(`🔒 Доступ ограничен списком из ${ALLOWED_EMAILS.size} e-mail.`);
}

// -----------------------------------------------------------------
// Защита: приложение обязано слать Firebase ID-токен в заголовке
// Authorization: Bearer <token>. Токен получают через Google Sign-In
// (firebase_auth на клиенте) — живёт час, подписан Google, нечего
// извлекать статическим анализом APK. req.uid доступен во всех
// хендлерах после этого middleware — используется для rate-limit.
// -----------------------------------------------------------------
async function checkFirebaseAuth(req, res, next) {
  ...
  try {
    ...
    if (ALLOWED_EMAILS.size > 0 && !ALLOWED_EMAILS.has(email)) {
      console.warn(`🔒 Отклонён посторонний e-mail: ${email || '(нет email в токене)'}`);
...
// =================================================================
// 📍 ПРОКСИ К GOOGLE PLACES — поиск точек рядом (голосовой Макс)
// =================================================================
// Ключ Google Maps/Places теперь живёт только тут — приложение
// шлёт только координаты и что искать, ответ Google пробрасываем
// как есть (формат тот же, что был при прямом вызове из приложения,
// клиентский код парсинга результатов не меняется).
app.get('/api/maps/nearby-search', checkFirebaseAuth, async (req, res) => {
  try {
    if (!GOOGLE_MAPS_API_KEY) {
      return res.status(500).json({ error: 'maps_key_not_configured' });
    }
    const { lat, lon, keyword } = req.query;
    if (!lat || !lon || !keyword) {
      return res.status(400).json({ error: 'lat, lon, keyword required' });
    }
